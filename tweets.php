...
<?php
	...
	function getRating($rating, $num){
		...
		global $nameid;
                ...
                switch($rating){
                        ...
                        case '2': echo "Not Unprofessional";
                        ...
                        case '3': echo "Unprofessional";
                                $sql = "insert into rating (tweetid, rating, raterid) values (".$tid.", 'Unprofessional', ".$nameid.")";
                                try{
                                        $conn->exec($sql);
                                }catch(PDOException $ex){
                                        echo $ex->getMessage();
	...
	if (isset($_POST["reload"])){
		...
		$url = "Location: http://grevera.ddns.net/~km677788/tweets.php?name=".$_POST["name"];
		header($url);
	}
	...
	if (isset($_POST["submit"])){
		...
		printf('<input id="name" name="name" type="hidden" value="%s">', $name);
	...
	echo "<h4>Help me rate these tweets for my research project!".
		...
		"as to whether each tweet is Unprofessional or NOT Unprofessional. ".
	...
	if(!isset($_GET["name"])){
		...
		echo "<h4>Thanks for rating more, ".$_GET["name"]."!</h4>";
	}

	$sql = "select * from  tweets where tweetid NOT IN (SELECT tweetid from rating) order by rand() limit 10";
	$statement = $conn->query($sql);
	...
	case "1":
		...
		<select name="rating1">
                        ...
                        <option value="2">Not Unprofessional</option>
